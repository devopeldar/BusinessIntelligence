  ...
  useEffect(() => {
    ...
    const GetTarea = async () => {
      try {
        ...
        setRolesxTarea(data.tareaxRolDTOs);
        //console.log("setRolesxTarea", data.tareaxRolDTOs);

        const datosMapeados = data.tareaxRolDTOs.map((item) => ({
            id: item.idTareaxRol,  // Ajusta esto según la estructura real de tus datos
            idUsuario: item.idUsuario,
            usuario: { /* ...estructura de usuario... */ },
            idRol: item.idRol,
            rol: { /* ...estructura de rol... */ },
          }));
          console.log("datosMapeados:", datosMapeados);

CargarDatos(datosMapeados);
          //setRolesxTareaUpdate(datosMapeados);
          //setRolesxTareaUpdate((prevDatos) => [...prevDatos, ...datosMapeados]);
...
//   useEffect(() => {
//     console.log("rolesxTarea después:", rolesxTarea);

//     const nuevosDatos = rolesxTarea.map((item) => ({
//       id: rolesxTarea.length + 1,
//       idUsuario: item.idUsuario, // Ajustar esto según la estructura real del objeto
//       nombreUsuario: item.nombreUsuario,
//       idRol: item.idRol, // Ajustar esto según la estructura real del objeto
//       nombreRol: item.nombreRol,
//     }));
//     console.log("nuevosDatos", nuevosDatos);
//     //setRolesxTareaUpdate(nuevosDatos);
//     // setRolesxTareaUpdate((prevDatos) => [...prevDatos, ...nuevosDatos]);
//     //   console.log("rolesxTarea después de la actualización:", rolesxTareaUpdate);

//     setRolesxTareaUpdate((prevDatos) => {
//       console.log("rolesxTarea después de la actualización:", prevDatos);
//       return [...prevDatos, ...nuevosDatos];
//     });
//   }, [rolesxTarea]);
  ...
  const handleAddRol = () => {
    ...
    const usuarioExistente = rolesxTarea.find(
      (item) =>
    ...
    if (!usuarioExistente) {
      setRolesxTarea((prevDatos) => [...prevDatos, newRow]);
    }
  };
  ...
  const eliminarItem = (id) => {
    const newData = rolesxTarea.filter((item) => item.id !== id);
    setRolesxTarea(newData);
  };
//   useEffect(() => {
//     console.log(
      ...
      <Card>
        ...
        >
          ...
          <MDTypography display="block" variant="button" color="white" my={1}>
            Un Tarea especifica las tareas que tienen que cumplir los encargados
            de las mismas
          </MDTypography>
        </MDBox>
                          ...
                          rolesxTareaUpdate.map((item) => (
                            <TableRow key={item.id}>
                              ...
                              <TableCell>
                                <IconButton
                                  aria-label="Eliminar"
                                  onClick={() => eliminarItem(item.idTareaxRol)}
                                >
                                  <Delete color="error" />
                                </IconButton>
                              </TableCell>
